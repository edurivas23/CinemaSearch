...
export const Hero = () => {
  ...
  const movieGenres =
    genres?.filter((genre) =>
      randomMovies[currentMovie]?.genre_ids?.includes(genre.id),
    ) ?? [];
  ...
  return (
    <section className="w-full mb-10 ">
      <div className=" relative  w-full h-180 ">
        <img
          key={currentMovie}
          src={`${backdropUrl}${randomMovies[currentMovie]?.backdrop_path}`}
          alt="Hero Image"
          className=" object-cover h-full w-full  mask-b-from-60% mask-b-to-100%  animate-fade-in-out"
        />
        ...
        <div className=" absolute bottom-30 mx-8 px-5 py-6  text-wrap max-w-3xl  bg-black/50  rounded-lg shadow-xl shadow-purple-950/50 ">
          ...
          <div className="text-yellow-300 text-md flex justify-between items-center">
            ...
            <Link
              ...
              className="text-white  bg-purple-950 rounded-md cursor px-4 py-2.5 shadow-lg shadow-black/40 cursor-pointer hover:bg-purple-900 transition ease-in"
            >
              Leer Más{" "}
            </Link>
          </div>
