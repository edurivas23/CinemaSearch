...
export const CardMovie = ({ movie, genres }: CardMovieProps) => {
  const { title, release_date, vote_average, poster_path, genre_ids, id } =
    movie;
  ...
  return (
    <Link
      ...
      className="bg-purple-950/20 rounded-lg shadow-xl/20  hover:ring-2 hover:ring-white transition-all duration-100 ease-out "
    >
      <article className=" h-full flex flex-col  text-white animate-fade-in delay-75  ">
        <div className="flex h-80 w-full">
          <img
            ...
            alt={title}
            className="object-fit w-full rounded-t-lg"
          />
        </div>
